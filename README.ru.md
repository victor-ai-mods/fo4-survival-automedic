...
Короткое описание на английском — [`README.md`](README.md).
...
5. Один раз за всю игру кладёт предмет AutoMedic в инвентарь. Если его потерять, вернуть
   можно кнопкой в MCM.
6. Запускает таймеры авторежима и Рад-Х, подписывается на `OnSit` и `OnItemEquipped` игрока и на открытие `SleepWaitMenu`.
...
- игрок мёртв или истекает кровью;
- игрок в сцене (диалог) — вне боя и не дольше 30 с подряд: застрявшая сцена авторежим не останавливает;
...
- **Травяные снадобья** (антимикробное, стимулирующее, болеутоляющее).
  ...
  - Когда:
    - `HerbalsBeforeSleep` — при открытии меню сна (`SleepWaitMenu`), если только что
      активированный объект (`GardenOfEden2.GetLastActivateTargetRef`) или занятая мебель
      (`GardenOfEden3.CurrentFurnitureHasKeyword`) несёт `IsSleepFurniture`, то есть до
      броска на болезнь при засыпании. Меню открывается раньше, чем игрок ложится, поэтому
      одной «занятой мебели» мало (2026-09-24). `OnSit` на кровать в игре не приходит (2026-09-24),
      он остался только строкой в логе;
...
|---|---|---|
| `README.md` | короткое описание на английском (главная страница репозитория) | актуален |
